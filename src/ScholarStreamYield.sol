...
contract ScholarStreamYield is ReentrancyGuard {
  ...
  function effectiveBalance(address payer) public view returns (uint256) {
    uint256 vaultValue = (vaultShares[payer] * vault.getPricePerShare()) /
      SCALE;
  ...
  function updatePayerBalances(address payer) public {
    ...
    require(
      effectiveBalance(payer) >= totalNeedToPay,
      "Insufficient funds for stream"
    );
  ...
  function rebalance(uint256 targetRatio) external nonReentrant {
    ...
    uint256 vaultValue = (vaultShares[msg.sender] * vault.getPricePerShare()) /
      SCALE;
    ...
    if (currentVaultRatio > targetRatio) {
      ...
      uint256 sharesToWithdraw = (excessAmount * SCALE) /
        vault.getPricePerShare();
      require(
        vaultShares[msg.sender] >= sharesToWithdraw,
        "Not enough vault shares for rebalance"
      );
      vaultShares[msg.sender] -= sharesToWithdraw;
      ...
      require(
        ...
        "Not enough direct funds for rebalance"
      );
      directBalances[msg.sender] -= shortageAmount;
