...
contract ScholarStream is ReentrancyGuard {
  // streamId => start time
  ...
  uint256 private constant RATE_LIMIT_DENOMINATOR =
    10_000_000_000 * 1_000 * 365 days * 1_000_000_000;
  ...
  function createStream(
    ...
    unchecked {
      require(
        amountPerSec < type(uint256).max / RATE_LIMIT_DENOMINATOR,
        "no overflow"
      );
    }
  ...
  function _withdraw(address from, address to, uint256 amountPerSec) internal {
    ...
    require(
      ...
      "Insufficient funds in paid balance"
    );
    paidBalance[from] -= paymentDue;
