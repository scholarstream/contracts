...
contract ScholarStreamYieldFactory {
    ...
    mapping(address=>mapping(address => ScholarStreamYield)) public payContracts;
    uint256 public payContractsArrayLength;

    event ScholarStreamCreated(address token, address vault,address scholarStream);

    function createPayContract(address _token, address _vault) external returns (ScholarStreamYield newContract) {
    ...
    function getPayContract(address _token, address _vault) external view returns (ScholarStreamYield) {
