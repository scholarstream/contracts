...
contract ScholarStreamYieldTest is Test {
  ...
  function testDeposit() public {
    ...
    assertEq(
      effectiveBal,
      depositAmount,
      "Effective balance should equal deposit amount"
    );
  ...
  function testWithdrawStreamAfterOneDay() public {
    ...
    console.log(
      "Remaining effective balance after withdrawals:",
      remainingEffective
    );
    assertLt(
      remainingEffective,
      depositAmount,
      "Remaining effective balance should be less than deposit"
    );
  }
  ...
  function testWithdrawPayer() public {
    ...
    assertEq(
      ...
      streamed,
      "Payee balance incorrect after withdrawal"
    );

    uint256 remainingEffective = scholar.effectiveBalance(payer);
    console.log(
      "Remaining effective balance before withdrawPayer:",
      remainingEffective
    );
    assertEq(
      remainingEffective,
      depositAmount - streamed,
      "Incorrect remaining effective balance"
    );
    ...
    assertEq(
      finalPayerBalance,
      initialBalance - streamed,
      "Payer did not receive correct withdrawal amount"
    );
    assertEq(
      scholar.effectiveBalance(payer),
      0,
      "Effective balance not zero after withdrawal"
    );
  }
  ...
  function testRebalance() public {
    ...
    uint256 vaultValue = (scholar.vaultShares(payer) *
      vault.getPricePerShare()) / scholar.SCALE();
    ...
    vaultValue =
      (scholar.vaultShares(payer) * vault.getPricePerShare()) /
      scholar.SCALE();
    ...
    assertApproxEqAbs(
      ...
      targetRatio,
      2,
      "Rebalance did not achieve target ratio"
    );
  }
  ...
  function testFullFlow() public {
    ...
    scholar.deposit(depositAmount, percentToVault);
    console.log(
      "Effective balance after deposit:",
      scholar.effectiveBalance(payer)
    );
    ...
    console.log(
      "Payee balance after first withdrawal:",
      token.balanceOf(payee)
    );
    ...
    uint256 remainingEffective = scholar.effectiveBalance(payer);
    console.log(
      "Remaining effective balance after cancellation:",
      remainingEffective
    );
    ...
    uint256 finalPayerBalance = token.balanceOf(payer);
    console.log(
      "Final payer token balance after withdrawal:",
      finalPayerBalance
    );
    assertEq(
      scholar.effectiveBalance(payer),
      0,
      "Effective balance should be zero after full withdrawal"
    );
  }
  ...
  function testYieldAccrual() public {
    ...
    assertGt(
      effectiveAfter,
      effectiveBefore,
      "Effective balance did not increase after yield"
    );
  }
}
